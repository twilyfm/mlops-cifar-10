...
class BasicNet(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=12, kernel_size=5, stride=3, dilation=3, padding=1),
            ...
            nn.BatchNorm2d(num_features=32),
            nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=3, dilation=3, padding=1),
            nn.BatchNorm2d(num_features=32)
        )
...
def train(model, optimizer, n_epochs, train_loader, val_loader, scheduler=None):
    ...
    for epoch in range(n_epochs):
        train_loss, train_acc = train_epoch(model, optimizer, loss_func, train_loader)
